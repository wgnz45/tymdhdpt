...
| 目标 SDK | API 36 |
| 前端打包 | `dist/` 直接打进 APK |
| API 地址（联调）| `http://192.168.15.218:3366`（写死在 `src/main.jsx`）|
...
5. APP 启动后应该能正常加载页面（数据走 `192.168.15.218:3366` 联网）
...
可能原因：
1. **API 不通**：手机访问不到 `192.168.15.218:3366`。在手机浏览器试试 `http://192.168.15.218:3366/api/store/default`，能返回数据吗？
